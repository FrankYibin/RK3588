    ...
    else
    {
        for(int i = 0; i < m_LocalFiles.size(); i++)
        {
            ...
            if(bResult == false)
                break;
            targetDirectory += ".txt";
			ExportToTxt(localAppDirectory, m_USBDirectory + "/" + targetDirectory);
...
bool TextExportWorker::ExportToTxt(const QString localDirectory, const QString targetDirectory)
{
    bool bResult = false;
    // 创建 QProcess 对象
    QProcess process;
    ...
    arguments.append("ConvertTxt.py");
