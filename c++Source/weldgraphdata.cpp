...
WeldGraphData* WeldGraphData::m_pInstance = nullptr;
...
/**************************************************************************//**
*
* \brief To get WeldGraphData singleton object.
*         The class is designed to the singleton class so the WeldGraphData object can used in the everywhere
*         without any new instantiation.
*         And the WeldGraphData object can be get easily using the this function
*         without any external declaration before using.
*
* \param none
*
* \return WeldGraphData singleton object
*
******************************************************************************/
WeldGraphData *WeldGraphData::getInstance()
{
    if(m_pInstance == nullptr)
    {
        m_pInstance = new WeldGraphData(nullptr);
    }
    return m_pInstance;
}
...
/**************************************************************************//**
*
* \brief It is the destructor for the WeldGraphData to release the WeldGraphData singleton object
*
* \param none
*
* \return none
...
int WeldGraphData::receiveWeldGraphData(double* receiveData, int length)
{
    ...
    axisMinValues.replace(GraphAxisEnum::VELOCITY_IDX, (uint32_t) - 1);
    ...
    axisMaxValues.replace(GraphAxisEnum::VELOCITY_IDX, 0);
    ...
    try
    {
        ...
        else
        {
            ...
            for(ind = 0 ; ind < totalEntriesPerParams ; ind++)
            {
                ...
                tmpVelocity = weldGraphResBuff[ind * PARAMETER_COUNT_PER_GRAPH_SAMPLE + 6];
                ...
                calculateLargest(axisMaxValues[GraphAxisEnum::VELOCITY_IDX], (qreal)tmpVelocity);
                calculateSmallest(axisMinValues[GraphAxisEnum::VELOCITY_IDX], (qreal)tmpVelocity);
