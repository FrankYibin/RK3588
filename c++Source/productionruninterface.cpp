...
*
******************************************************************************/
ProductionRun::ProductionRun(QObject *parent)
    : QThread(parent)
...
*
******************************************************************************/
void ProductionRun::slotHeartbeatUpdated(void* _obj)
{
    HeartbeatFormat* _heatbeatObj = reinterpret_cast<HeartbeatFormat*>(_obj);
    updateActiveRecipeNum(_heatbeatObj->m_activeRecipeNum);
    updateCycleCount(QString::number(_heatbeatObj->m_cycleNum));
    updateTotalCycleTime(QString::number(_heatbeatObj->m_cycleTime));
    updatePeakPowerRatio(_heatbeatObj->m_powerValue / 100.00);
}
